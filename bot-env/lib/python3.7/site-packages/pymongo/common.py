...
def validate_ok_for_update(update):
    """Validate an update_wallet document."""
    validate_list_or_mapping("update_wallet", update)
    # Update cannot be {}.
    if not update:
        raise ValueError('update_wallet cannot be empty')

    is_document = not isinstance(update, list)
    first = next(iter(update))
    if is_document and not first.startswith('$'):
        raise ValueError('update_wallet only works with $ operators')
